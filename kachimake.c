...
int judgeVertical(int board[BOARD_SQUARE][BOARD_SQUARE], int x, int y)
{
    ...
    int i, x2 = x-1, y2 = y-2;
    // x-1, y-1から上方向，下方向に順番に何個並ぶか確認していく
...
int judgeWidth(int board[BOARD_SQUARE][BOARD_SQUARE], int x, int y)
{
    ...
    int i, x2 = x-2, y2 = y-1;
    // x-1, y-1から左方向，右方向に順番に何個並ぶか確認していく
    ...
    if(cnt==5) return 1;
    // 右方向に何個並ぶか確認。最大4�?([x-1][y-1]を除く�?�で)
    // 仮にboardに何もな�?ところは0、�?��?が置�?たところは1、相手が置�?たところは2で格納されて�?るとする
    x2 = x;
    for(i=0;i<4;i++)
    {
...
{
    int cnt = 1; // 並んでる数をカウントす�?
    // x-1, y-1から右斜め、左斜めの�?番に何個並ぶか確認して�?くい�?
    // 左下に何個並ぶか確認。最大4�?([x-1][y-1]を除く�?�で)
    // 仮にboardに何もな�?ところは0、�?��?が置�?たところは1、相手が置�?たところは2で格納されて�?るとする
    int i, x2 = x-2, y2 = y;
    for(i=0; i<4;i++)
    {
