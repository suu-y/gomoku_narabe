...
{
    if(judgeWin(board, x, y)) strcat(message, ",win");
}
...
int judgeVertical(int board[BOARD_SQUARE][BOARD_SQUARE], int x, int y)
{
    ...
    // 上方向に何個並ぶか確認。最大4個([x-1][y-1]部分を除くので)
    // 仮にboard内に何もないところは0，自分が置いたところは1，相手が置いたところは2で格納されているとする
    for(i=0; i<4;i++)
    {
    ...
    if(cnt==5) return 1;
    // 下方向に何個並ぶか確認。最大4個([x-1][y-1]部分を除くので)
    // 仮にboard内に何もないところは0，自分が置いたところは1，相手が置いたところは2で格納されているとする
    y2 = y;
    for(i=0;i<4;i++)
    {
...
int judgeWidth(int board[BOARD_SQUARE][BOARD_SQUARE], int x, int y)
{
    ...
    // 左方向に何個並ぶか確認。最大4個([x-1][y-1]部分を除くので)
    // 仮にboard内に何もないところは0，自分が置いたところは1，相手が置いたところは2で格納されているとする
    for(i=0; i<4;i++)
    {
        if(board[x2--][y2]==1) cnt++;
    ...
    if(cnt==5) return 1;
    // 右方向に何個並ぶか確認。最大4個([x-1][y-1]部分を除くので)
    // 仮にboard内に何もないところは0，自分が置いたところは1，相手が置いたところは2で格納されているとする
    x2 = x;
    for(i=0;i<4;i++)
    {
...
{
    int cnt = 1; // 並んでいる数をカウントする
    // x-1, y-1から右斜め，左斜めの順番に何個並ぶか確認していく
    // 左下に何個並ぶか確認。最大4個([x-1][y-1]部分を除くので)
    // 仮にboard内に何もないところは0，自分が置いたところは1，相手が置いたところは2で格納されているとする
    int i, x2 = x-2, y2 = y-2;
    for(i=0; i<4;i++)
    {
